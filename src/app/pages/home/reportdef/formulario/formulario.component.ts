...
export class FormularioComponent implements OnInit {
  ...
  @Output() editarOrAltaAbm = new EventEmitter<FormdataReportdef>();
  ...
  async ngOnInit() {
    ...
    let formAux = JSON.parse(localStorage.getItem("photoAbm"));
    let photo = false;
    if (formAux) {
      localStorage.removeItem("photoAbm");
      localStorage.setItem("photoAbmDef", JSON.stringify(formAux));

    } else {
      formAux = JSON.parse(localStorage.getItem("photoAbmDef"));
      if (formAux) {
        localStorage.removeItem("photoAbmDef");
        photo = true;
  ...
  continuaProcess(hijo: boolean, event: FormdataReportdef) {
    ...
    if (event.buttomDTO.metodoDTO.tipoMetodo.toUpperCase() === FrontEndConstants.LLAMAR_URL_EXTERNA.toUpperCase()) {
      ...
      this.router.navigate(['/externalUrlMaps', {
        data: '/SFSFrameworkRest/maps/mapa.jsp',
  ...
  processEditarAbm(event: FormdataReportdef) {
    this.doPhoto(this.data, this.form,event);
    this.editarOrAltaAbm.emit(event);
  }
  verFiltros() {
    this.filtros = true;
  }

  private doPhoto(data: any, form: FormGroup, photo: FormdataReportdef) {
    const listNew: FormdataReportdef[] = [];

    for (const f of data.list) {
      if ((f.busquedaGenerica || f.buttom || f.upload || f.cards || f.uploadImpresion || f.autocomplete) && !f.combo) {
        if(f.name === photo.name){
          f.photo=true;
        }
        listNew.push(f);
        continue;
    ...
    localStorage.setItem("photoAbm", JSON.stringify(listNew));
  }
   async actualizarDesdePhoto(data: FormdataReportdef[],photos: FormdataReportdef[]) {

    for (const f of data) {
      for (const photo of photos) {
        if (f.name === photo.name) {
          f.value = photo.valueNew;
          f.valueNew = photo.valueNew;
          if (f.entity) {
            f.busquedaGenericaDTO.mostrarToStringLupa = photo.busquedaGenericaDTO.mostrarToStringLupa;
            if(f.valueNew && (f.busquedaGenericaDTO.mostrarToStringLupa===null ||f.busquedaGenericaDTO.mostrarToStringLupa===undefined || f.busquedaGenericaDTO.mostrarToStringLupa ==='' )){
              await this.refrescarToString(f);
              console.log(f);
            }
          }
        }
 ...
 refrescarToString(event:FormdataReportdef){
  return new Promise<void>(resolve => {
    const user = JSON.parse(localStorage.getItem('currentUser'));
  ...
  descEntidad.name = event.type;
  descEntidad.id = event.valueNew;
    ...
    result => {
      console.log(result);
        for (const p1 of result.listToString) {
          if (event.type === p1.name) {
            event.busquedaGenericaDTO.mostrarToStringLupa = p1.value;
            resolve();
          }
      };
    });
